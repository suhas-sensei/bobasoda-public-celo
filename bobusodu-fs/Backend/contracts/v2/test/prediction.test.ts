...
const MIN_BET_AMOUNT = ether("1"); // 1 ETH
...
contract(
  ...
  ([operator, admin, owner, bullUser1, bullUser2, bullUser3, bearUser1, bearUser2, bearUser3]) => {
    ...
    it("Should record data and user bets", async () => {
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("1.1") }); // 1.1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("1.2") }); // 1.2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("1.4") }); // 1.4 ETH

      assert.equal((await balance.current(prediction.address)).toString(), ether("3.7").toString()); // 3.7 ETH
      assert.equal((await prediction.rounds(1)).totalAmount, ether("3.7").toString()); // 3.7 ETH
      assert.equal((await prediction.rounds(1)).bullAmount, ether("2.3").toString()); // 2.3 ETH
      assert.equal((await prediction.rounds(1)).bearAmount, ether("1.4").toString()); // 1.4 ETH
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("2.1") }); // 2.1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("2.2") }); // 2.2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("2.4") }); // 2.4 ETH

      assert.equal((await balance.current(prediction.address)).toString(), ether("10.4").toString()); // 10.4 ETH (3.7+6.7)
      assert.equal((await prediction.rounds(2)).totalAmount, ether("6.7").toString()); // 6.7 ETH
      assert.equal((await prediction.rounds(2)).bullAmount, ether("4.3").toString()); // 4.3 ETH
      assert.equal((await prediction.rounds(2)).bearAmount, ether("2.4").toString()); // 2.4 ETH
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("3.1").toString() }); // 3.1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("3.2").toString() }); // 3.2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("3.4").toString() }); // 4.3 ETH

      assert.equal((await balance.current(prediction.address)).toString(), ether("20.1").toString()); // 20.1 ETH (3.7+6.7+9.7)
      assert.equal((await prediction.rounds(3)).totalAmount, ether("9.7").toString()); // 9.7 ETH
      assert.equal((await prediction.rounds(3)).bullAmount, ether("6.3").toString()); // 6.3 ETH
      assert.equal((await prediction.rounds(3)).bearAmount, ether("3.4").toString()); // 3.4 ETH
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("4.1").toString() }); // 4.1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("4.2").toString() }); // 4.2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("4.4").toString() }); // 4.4 ETH

      assert.equal((await balance.current(prediction.address)).toString(), ether("32.8").toString()); // 32.8 ETH (3.7+6.7+9.7+12.7)
      assert.equal((await prediction.rounds(4)).totalAmount, ether("12.7").toString()); // 12.7 ETH
      assert.equal((await prediction.rounds(4)).bullAmount, ether("8.3").toString()); // 8.3 ETH
      assert.equal((await prediction.rounds(4)).bearAmount, ether("4.4").toString()); // 4.4 ETH
    ...
    it("Should record rewards", async () => {
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("3.1").toString() }); // 3.1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("3.2").toString() }); // 3.2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("3.4").toString() }); // 3.4 ETH

      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 ETH, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 ETH, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, 0);
      assert.equal((await prediction.rounds(2)).rewardAmount, 0);
      assert.equal(await prediction.treasuryAmount(), ether("3.7") * INITIAL_TREASURY_RATE); // 3.7 ETH, Total * treasuryRate
      assert.equal(
      ...
      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 ETH, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 ETH, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, ether("2.4").toString()); // 2.4 ETH, Bear total
      assert.equal((await prediction.rounds(2)).rewardAmount, ether("6.7") * INITIAL_REWARD_RATE); // 6.7 ETH, Total * rewardRate
    ...
    it("Should claim rewards", async () => {
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("1") }); // 1 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("2") }); // 2 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("4") }); // 4 ETH
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("21") }); // 21 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("22") }); // 22 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("24") }); // 24 ETH
    ...
    it("Should multi claim rewards", async () => {
      ...
      // 4.2 = 2/3 * (7*0.9) + // 30.851162790697674418 = 22 / 43 * (67 * 0.9) = 35.051162790697674418 ETH
    ...
    it("Should claim treasury rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await balance.current(prediction.address)).toString(), predictionCurrentETH.toString());
      ...
      await prediction.betBull(currentEpoch, { from: bullUser1, value: ether("31") }); // 31 ETH
      await prediction.betBull(currentEpoch, { from: bullUser2, value: ether("32") }); // 32 ETH
      await prediction.betBear(currentEpoch, { from: bearUser1, value: ether("34") }); // 34 ETH
      ...
      predictionCurrentETH = predictionCurrentETH.sub(ether("0.7"));
      assert.equal((await balance.current(prediction.address)).toString(), predictionCurrentETH.toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0); // Empty
      predictionCurrentETH = predictionCurrentETH.sub(ether("16.4"));
      assert.equal((await balance.current(prediction.address)).toString(), predictionCurrentETH.toString());
    });
